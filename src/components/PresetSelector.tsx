...
const PresetSelector: React.FC<PresetSelectorProps> = ({ setProcesses, resetSimulation }) => {
  ...
  const handlePresetChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (preset && preset.processes.length > 0) {
      ...
      const newProcesses: Process[] = preset.processes.map((template, index) => ({
        ...template,
        id: index + 1,
  ...
  return (
    <div className="preset-selector">
      ...
      <select id="preset-select" onChange={handlePresetChange} defaultValue={processPresets[0].name}>
