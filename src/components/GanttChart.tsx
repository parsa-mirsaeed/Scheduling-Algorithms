...
const GanttChart: React.FC<GanttChartProps> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const style = getComputedStyle(document.documentElement);
    const primaryColor = style.getPropertyValue('--primary-color').trim() || '#3f51b5';
    const secondaryColor = style.getPropertyValue('--secondary-color').trim() || '#00bcd4';
    const textPrimary = style.getPropertyValue('--text-primary').trim() || '#212121';
    const textSecondary = style.getPropertyValue('--text-secondary').trim() || '#757575';
    ...
    const uniqueProcessIds = [...new Set(data.map(item => item.processId))].sort((a, b) => a - b);
    ...
    data.forEach((item) => {
      ...
      ctx.quadraticCurveTo(x + width, yStart + barHeight, x + width - radius, yStart + barHeight);
      ...
      if (width > 50) {
        ctx.fillText(`P${item.processId}`, x + width / 2, yStart + barHeight / 2 - 8);
        ctx.font = '12px Inter, Roboto, sans-serif';
        ctx.fillText(`${item.endTime - item.startTime}`, x + width / 2, yStart + barHeight / 2 + 12);
      } else {
